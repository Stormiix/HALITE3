            ...
            if ship.position == me.shipyard.position:
                ...
                if len(dropoffs) > 0:
                    ...
                    for dropoff in dropoffs:
                        oldDistance = game_map.calculate_distance(
                            ship.position, min.position)
                        distance = game_map.calculate_distance(
                            ship.position, dropoff.position)
                        if oldDistance > distance:
                            min = dropoff
                    move = game_map.naive_navigate(ship, min)
                    command_queue.append(ship.move(move))
                    logging.info("Moving to dropoff !")
                    continue
                if random.randint(0, 100) > 70:
                    logging.info("Making a dropoff !")
                    command_queue.append(ship.make_dropoff())
                    continue
                else:
                    move = game_map.naive_navigate(ship, me.shipyard.position)
                    command_queue.append(ship.move(move))
                    logging.info("Moving to shipyard !")
                    continue
